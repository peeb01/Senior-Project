...
def ann_model(x_shape, y_shape):
    ...
    """
    model = Sequential()
    model.add(Dense(units=1000, input_shape=(x_shape[1],), activation=tf.keras.activations.relu))
...
def cnn_model(x_shape, y_shape):
    ...
    model.add(Conv1D(filters=64, kernel_size=3, input_shape=(x_shape[1], 1), activation=tf.keras.activations.relu))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=128, kernel_size=3, activation=tf.keras.activations.relu))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=256, kernel_size=3, activation=tf.keras.activations.relu))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Flatten())
